...
//                        .onErrorReturn(e -> reducers.pullToRefreshLoading(false, e)));
        ...
        Observable<Action> initData = Observable.just(Arrays.asList("1","2","3","4"))
                .map(list -> {
                    ListData<String> listData = new ListData<>();
                    listData.data = list;
                    return reducers.setData(listData,false);
                })
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io());
        Observable<PullAndLoadMoreViewState> stateObservable =
                transformState.dispatch(Observable.merge(initData,initData), store);
